...
def create_model(fingerprint_input, model_settings, model_architecture,
    ...
    if model_architecture == 'single_fc':
        ...
        return create_resnet_model(fingerprint_input, model_settings,
                                   is_training)
    # elif model_architecture == 'resnet_small':
    #     return create_resnet_small_model(fingerprint_input, model_settings, is_training)
    else:
...
def create_resnet_model(fingerprint_input, model_settings, is_training,
    ...
    # elif name == 'resnet':
    network = resnet_model.resnet_generator(label_count, dropout_prob,
                                            data_format='channels_last')
    # elif name == 'resnetft':
...
#     fingerprint_input = tf.reshape(fingerprint_input, [-1, 40, 98, 1])
#     logits = resnet.resnet.inference(fingerprint_input,
...
#
#
# def create_resnet_small_model(fingerprint_input, model_settings, is_training):
#     if is_training:
#         dropout_prob = tf.placeholder(tf.float32, name='dropout_prob')
#     label_count = model_settings['label_count']
#
#     import resnet.resnet
#     fingerprint_input = tf.reshape(fingerprint_input, [-1, 40, 98, 1])
#     logits = resnet.resnet.inference_small(fingerprint_input,
#                                            num_classes=label_count,
#                                            is_training=True)
#
#     if is_training:
